...
class P1Test(object):
    ...
    def run(self):
        for db in self.conf.databases:
            gdb = self.conf.make_db(database=db, dataset=self.conf.test_dataset)
            if gdb is None:
                continue
            self.run_one(gdb)
    ...
    def validate_empty_edges(self, gdb):
        assert (
            gdb.number_of_edges() == 0
        ), f"number_of_edges must be =0: {gdb.number_of_edges()}"

    def validate_empty_nodes(self, gdb):
        assert (
            gdb.number_of_nodes() == 0
        ), f"number_of_nodes must be =0: {gdb.number_of_nodes()}"

    def validate_contents(self, gdb):
        for e in self.edges:
            assert gdb.has_edge(e.first, e.second), f"No directed edge: {e}"

        assert gdb.number_of_edges() == 10, f"number_of_edges: {gdb.number_of_edges()}"
        assert gdb.number_of_nodes() == 8, f"number_of_nodes: {gdb.number_of_nodes()}"
        assert gdb.reduce_edges(None, 1) == GraphDegree(
            3, 10.0
        ), f"reduce_edges: {gdb.reduce_edges(None, 1)}"
        assert gdb.reduce_edges(1, None) == GraphDegree(
            1, 4.0
        ), f"reduce_edges: {gdb.reduce_edges(1, None)}"
        assert gdb.reduce_edges(1, 1) == GraphDegree(
            4, 14.0
        ), f"reduce_edges: {gdb.reduce_edges(1, 1)}"
        assert set(gdb.neighbors(1)) == {2, 4, 6, 7}, f"neighbors: {gdb.neighbors(1)}"
        assert set(gdb.neighbors_of_neighbors(8)) == {
            1
        }, f"neighbors_of_neighbors: {gdb.neighbors_of_neighbors(8)}"
        assert gdb.reduce_edges(None, 5) == GraphDegree(
            1, 3.0
        ), f"reduce_edges: {gdb.reduce_edges(None, 5)}"
        assert gdb.reduce_edges(5, None) == GraphDegree(
            1, 2.0
